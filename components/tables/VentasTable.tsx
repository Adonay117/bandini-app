...
export function VentasTable({ ventas }: { ventas: Venta[] }) {
  return (
    <ul className="flex flex-col gap-2">
      {ventas.map((v) => {
        const unidades = v.items.reduce((s, i) => s + i.cantidad, 0);
        const descuentoTotal = v.items.reduce((s, i) => s + i.descuento, 0);
        return (
          <li
            key={v.id}
          ...
          >
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium text-ink">
                {v.items.map((i) => i.producto_nombre ?? 'Producto eliminado').join(', ')}
              </p>
            ...
            <div className="shrink-0 text-right">
              ...
              {descuentoTotal > 0 && (
                <p className="text-[11px] text-warning tabular-nums">−{formatCurrency(descuentoTotal)} desc.</p>
              )}
            </div>
          </li>
        );
